...
| `status_bar.shader` | string | `pulse` | Effect drawn over the bar while a turn runs. `blaze`, `burn`, `inferno`, `noise`, `shine` and `trippy` recolor only the text, leaving background colors and glyphs such as the `█▓▒░` fade as the layout drew them. `pulse` gently brightens the whole bar, `fade` fades the text in from the background color, and `grayFade` drains the color from it. An empty string leaves the bar unshaded. |
